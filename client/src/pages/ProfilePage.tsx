...
const ProfilePage: React.FC = () => {
  ...
  const handleBlockToggle = async () => {
    ...
    try {
      // ИСПРАВЛЕНО: Передаем объект с инвертированным статусом
      await dispatch(
        toggleBlockUser({
          id: user.id,
      ...
      // Обновляем данные пользователя (хотя slice уже обновит currentUser,
      // getMe лишний раз подстрахует)
      dispatch(getMe());

      setNotification({
        open: true,
        message: user.isActive
          ? "Аккаунт заблокирован" // Если был активен (true), то теперь заблокирован
          : "Аккаунт разблокирован", // Если был заблокирован (false), то теперь разблокирован
        type: "success",
      });
    } catch (err: any) {
      setNotification({
