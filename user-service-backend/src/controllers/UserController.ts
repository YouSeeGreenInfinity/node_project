...
export class UserController {
  ...
  static async toggleUserBlock(req: Request, res: Response): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      const statusCode = errorMessage.includes('не найден') ? 404 : 
                        errorMessage.includes('Нельзя заблокировать') ? 403 : 400;
  ...
  static async changePassword(req: Request, res: Response): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      const statusCode = errorMessage.includes('не найден') ? 404 : 
                        errorMessage.includes('неверен') ? 400 : 500;
  ...
  static async deleteUser(req: Request, res: Response): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      const statusCode = errorMessage.includes('не найден') ? 404 : 
                        errorMessage.includes('Нельзя удалить') ? 403 : 500;
