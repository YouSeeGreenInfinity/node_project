...
+ 1. Инициализация проекта + создание репозитория
   ```bash
...
4. **Структура проекта (создать папки)**
   ```
   src/
   ├── config/           # Конфигурация
...
3. **Создать вспомогательные модели/интерфейсы** (`src/types/`):
...
1. **Создать основной файл приложения** (`src/app.ts`)
   - Настроить middleware (json, cors, logging)
   - Подключить базу данных
   - Создать тестовый маршрут

2. **Создать точку входа** (`src/server.ts`)
   - Запуск сервера
